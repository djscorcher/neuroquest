...
export const syncProfile = async (userId, { playerName, xp, themeKey, streak = 0, completedCount = 0, gems = 0, titles = [], bossState = null, streakState = null, trialState = null, notifyPrefs = null, tz = null }) => {
  ...
  if (streakState)      payload.streak_state = streakState;
  if (trialState)       payload.trial_state  = trialState;
  if (notifyPrefs)      payload.notify_prefs = notifyPrefs; // requires notify_prefs jsonb column in profiles table
  if (tz)               payload.tz           = tz;          // requires tz text column in profiles table
